...
\theorem{Any finite set of 30 or more points in the plane in general position contains a $6$-hole.}
\begin[Proof outline]{proof}
\begin{itemize}
  Fix a point $S$.

  Every point set gives rise to an assignment of \emph{triple orientations}~(\Cref{sec:triple-orientations}).


  Suppose for a contradiction that $S$ did not contain a $6$-hole.
  It is true that 

  Then 
  By contradiction.
  Suppose that $S$ did *not* contain a $6$-hole.
  Then it would 
...
We begin in~\Cref{sec:geometry} by introducing geometric aspects of the problem
and stating our main result.


followed by a discussion of \emph{triple orientations}





\Cref{sec:geometry} discusses the basic geometric aspects of the problem.
Then,~\Cref{sec:triple-orientations} is devoted to \emph{triple orientations},
...
Next,~\Cref{sec:properties-of-sigma} proves properties of orientations
that reduce the search space,
%corresponding to~\Cref{eq:signotopeClauses11,eq:signotopeClauses12}.

The Empty Hexagon Number requires a sophisticated encoding,
presented in~\Cref{fig:full-encoding},
consisting of two main ingredients.
First,~\Cref{sec:symmetry-breaking} deals with symmetry breaking,
proving that one can assume, without loss of generality,
that points are in \emph{canonical position},
a definition that includes points being simultaneously sorted from left-to-right
and by angle with respect to the leftmost point.
Then,~\Cref{sec:empty-hexagon-number} deals with the efficient re-encoding of Heule and Scheucher,
the crucial piece to make the proof of $h(6) = 30$ feasible.
Next,~\Cref{sec:leansat} presents the preliminary \textsf{LeanSAT} library,
which is used to create a SAT encoding
that is provably connected to the geometric properties it describes.
...
% WN: TODO move this closer to where the variable names are explained
% and the satisfying assignment is defined
