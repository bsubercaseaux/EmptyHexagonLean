...
%    \vspace{0.5cm}
    ...
    \end{subfigure}
  \caption{The pointsets depicted in \Cref{fig:equiv-a,fig:equiv-b} are $\sigma$-equivalent with \lstinline|parity := false|
  since the bijection $f$ defined by $(a,b,c,d) \mapsto (b', d', c', a')$ satisfies $\sigma(p_i, p_j, p_k) = \sigma(f(p_i), f(p_j), f(p_j))$ for every $\{p_i, p_j, p_k\} \subseteq \{a,b,c,d\}$.
On the other hand, no orientation-preserving bijection exists for~\Cref{fig:equiv-c,fig:equiv-d},
which are only $\sigma$-equivalent with \lstinline|parity := true|.}\label{fig:sigma-equiv}
