...
but we do not (need to) prove completeness because these clauses are already unsatisfiable.}
...
such that $ace$ is a $3$-hole, but it turns out that this is enough to ensure
a $6$-hole by ``flattening'' the triangles $ape$, $edc$, and $cba$ if necessary
to obtain empty triangles, which can be assembled into a $6$-hole $ap'ed'cb'$.

Justifying this reasoning formally turned out to be complex,
