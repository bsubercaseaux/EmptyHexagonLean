...
open List Classical LeanSAT Point
attribute [-simp] getElem_fin

theorem satisfies_no6Hole3Below {w : CanonicalPoints} (hw : ¬σHasConvexKGon 6 w.toFinset)
    {a c d e : Fin w.rlen} (de : d < e) :
    (no6Hole3Below a c d e).eval (w.toPropAssn False) := by
  simp [no6Hole3Below]; intro _ ⟨b, ab, bc, cd, abc, bcd⟩ cde
  refine hw <| σCCWPoints.convexHexagon ?_ w.gp
...
theorem satisfies_no6Hole4Above {w : CanonicalPoints} (hw : ¬σHasConvexKGon 6 w.toFinset)
    {a d e f : Fin w.rlen} (ef : e < f) :
    (no6Hole4Above a d e f).eval (w.toPropAssn False) := by
  simp [no6Hole4Above]; intro ⟨de, c, ⟨b, ab, bc, cd, abc, bcd⟩, cde, _⟩
  refine (w.gp₃' de ef).σ_iff'.1 fun def_ => ?_
  refine hw <| (σCCWPoints.cycle (l₂ := [_, _, _, _, _]) ?_).convexHexagon
    w.gp (subset_map' w [f, e, d, c, b, a])
...
theorem satisfies_no6Hole2Below {w : CanonicalPoints} (hw : ¬σHasConvexKGon 6 w.toFinset)
    {a c e f : Fin w.rlen} :
    (no6Hole2Below a c f e).eval (w.toPropAssn False) := by
  ...
  refine hw <| this.convexHexagon w.gp (subset_map' w [a, b, c, f, e, d])

theorem satisfies_no6Hole1Below {w : CanonicalPoints} (hw : ¬σHasConvexKGon 6 w.toFinset)
    {a d e f : Fin w.rlen} (ae : a < e) (ef : e < f) :
    (no6Hole1Below a d f e).eval (w.toPropAssn False) := by
  simp [no6Hole1Below]; intro ⟨df, c, ⟨b, ab, bc, cd, abc, bcd⟩, cdf, _⟩ aef
  have := Arc.join (l₁ := [_]) (l₂ := [_,_,_])
    (.cons' ae aef <| .one' ef)
    (.cons' ab abc <| .cons' bc bcd <| .cons' cd cdf <| .one' df)
  refine hw <| this.convexHexagon w.gp (subset_map' w [a, e, f, d, c, b])

theorem satisfies_hexagonEncoding (w : CanonicalPoints) (hw : ¬σHasConvexKGon 6 w.toFinset) :
    (hexagonEncoding w.rlen).eval (w.toPropAssn False) := by with_reducible
...
theorem satisfies_hexagonEncoding' (w : CanonicalPoints) (hw : ¬σHasConvexKGon 6 w.toFinset) :
    (hexagonEncoding' w.rlen).eval (w.toPropAssn False) := by with_reducible
  simp [hexagonEncoding', satisfies_hexagonEncoding w hw, holeUnits, Var.hole]
