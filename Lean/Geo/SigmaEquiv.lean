...
theorem hσ (f : S ≃σ T) (p q r : S) : σ p q r = σ (f p) (f q) (f r) :=
  f.hσ' p q r
...
@[simp]
theorem symm_apply_apply (e : S ≃σ T) (s : S) : e.symm (e s) = s :=
  EquivLike.inv_apply_apply e s

@[simp]
theorem apply_symm_apply (e : S ≃σ T) (t : T) : e (e.symm t) = t :=
  EquivLike.apply_inv_apply e t
