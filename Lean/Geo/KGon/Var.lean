...
inductive Var (n : Nat) where
  ...
  | arc1   (o : Orientation) (sz : Nat) (a c d : Fin n)
  | capF   (a d e : Fin n)
...
@[match_pattern] def Var.cap (a b c : Fin n) := Var.arc1 .cw 0 a b c
@[match_pattern] def Var.cup (a b c : Fin n) := Var.arc1 .ccw 0 a b c
...
instance : Repr (Var n) where
  ...
  | .arc1 o sz a b c, _ => f!"arc1 {repr o} {sz} {a.succ} {b.succ} {c.succ}"
...
instance : LinearOrder (Var n) :=
  LinearOrder.lift'
    ...
    (fun
      ...
      | .arc1 o sz a c d => .inrₗ <| .inlₗ (o, sz, a, c, d)
...
def Var.toCode : Var n → String
  ...
  | .arc1 .cw sz a b c => s!"3 {sz} {a} {b} {c}"
  | .arc1 .ccw sz a b c => s!"4 {sz} {a} {b} {c}"
